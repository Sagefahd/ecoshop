...
from catalog.models import Category, Vendor, Product, Promo


class Command(BaseCommand):
    help = "Seeds the database with demo categories, a vendor, products, and a promo for local development."
    ...
    def handle(self, *args, **options):
        ...
        vendor, _ = Vendor.objects.get_or_create(
            user=vendor_user,
            defaults={"store_name": "Accra Mall Traders", "whatsapp_number": "233201234567"},
        )
        ...
        p1, _ = Product.objects.get_or_create(
            vendor=vendor, category=cat1, name="Bluetooth Headphones",
            defaults={"price": 250, "stock_quantity": 20,
                      "description": "Wireless over-ear headphones with noise cancellation."},
        )
        Product.objects.get_or_create(
            vendor=vendor, category=cat2, name="Kente Print Shirt",
            defaults={"price": 180, "stock_quantity": 4,
                      "description": "Locally made shirt with kente-inspired print."},
        )
        Product.objects.get_or_create(
            vendor=vendor, category=cat3, name="5kg Rice Bag",
